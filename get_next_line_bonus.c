...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/11/17 14:01:44 by rbozdemi          #+#    #+#             */
/*   Updated: 2022/11/17 16:51:07 by rbozdemi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	arr[fd] = split_line(fd, arr[fd]);
	if (!arr[fd])
		return (NULL);
	line = make_line(arr[fd]);
	arr[fd] = make_arr(arr[fd]);
	return (line);
}
